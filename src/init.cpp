...
std::string HelpMessage()
{
    ...
    strUsage += "  -conf=<file>           " + _("Specify configuration file (default: Bitmonk.conf)") + "\n";
    strUsage += "  -pid=<file>            " + _("Specify pid file (default: bitmonkd.pid)") + "\n";
...
strUsage += "\n" + _("Masternode options:") + "\n";
    ...
    strUsage += "  -anonymizebitmonkamount=<n> " + _("Keep N Bitmonk anonymized (default: 0)") + "\n";
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Bitmonk is shutting down."));
    ...
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Bitmonk is probably already running."), strDataDir));

    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("Bitmonk version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    if (fDaemon)
        fprintf(stdout, "Bitmonk server starting\n"); 
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Bitmonk") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Bitmonk to complete") << "\n";
    ...
    nAnonymizeBitmonkAmount = GetArg("-anonymizebitmonkamount", 0);
    if(nAnonymizeBitmonkAmount > 999999) nAnonymizeBitmonkAmount = 999999;
    if(nAnonymizeBitmonkAmount < 2) nAnonymizeBitmonkAmount = 2;
    ...
    LogPrintf("Anonymize Bitmonk Amount %d\n", nAnonymizeBitmonkAmount);
